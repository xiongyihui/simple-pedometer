...
/**
 * Receives sensor updates and alerts a StepListener when a step has been
 * detected.
 */
public class SimpleStepDetector {
	...
	public void updateAccel(long timeNs, float x, float y, float z) {
		...
		worldZ[0] = SensorFusionMath.sum(accelRingX)
				/ Math.min(accelRingCounter, ACCEL_RING_SIZE);
		...
		float currentZ = SensorFusionMath.dot(worldZ, currentAccel)
		...
		listener.probe(velocityEstimate);

		if (lastStatus != 1 && velocityEstimate > (STEP_THRESHOLD + 1)) {
			lastStatus = 1;

			upTimeNs = timeNs;
		} else if (lastStatus != 0 && (velocityEstimate < (-STEP_THRESHOLD))) {
			lastStatus = 0;

			downTimeNs = timeNs;
			long deltaNs = downTimeNs - upTimeNs;
			if (deltaNs < STEP_MAX_NS
